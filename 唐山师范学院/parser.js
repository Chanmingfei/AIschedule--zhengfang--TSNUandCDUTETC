...
function scheduleHtmlParser(html) {
							...
							$(this).children('p').each(function (inn, em) {
								...
								switch (text) {
									case '节/周':
										...
										break;
									case '上课地点': re.position = $(this).text().trim();
										break;
									case '教师': re.teacher = $(this).text().trim();
										break;
								}
							})
							result.push(re)
						...
						if (index1 > 0) {   //跳过星期列
							...
							sp_tr.find('p font').each(function (index2, em2) {
								switch ($(this).find('span').last().attr('class')) {
									...
									case 'glyphicon glyphicon-map-marker': re.position = $(this).text().trim();
										break;
									case 'glyphicon glyphicon-user': re.teacher = $(this).text().trim();
										break;
								}
							})
							console.log(re)
	...
	if (message.length != 0) {
		result.length = 0
		result.push({ name: "遇到错误,可联系开发者qq 2068393368 进行反馈", teacher: "开发者-陈铭霏", position: message, day: 1, weeks: [1], sections: [{ section: 1 }, { section: 2 }] })
	}
	console.info(result)
